...
public class QuizService {
    ...
    public QuizAttempt submitQuiz(Quiz quiz, List<Integer> studentAnswers, String studentId) {
        ...
        //if course passed generate a certificate
        if (passed) {
            String courseId = getCourseIdFromQuiz(quiz);
            if (courseId != null) {
                completeLessonAfterQuiz(studentId, courseId, quiz.getLessonId(), true);
                checkCourseCompletionAndGenerateCertificate(studentId, courseId);
            }
        }
        return attempt;
    }
    ...
    private void checkCourseCompletionAndGenerateCertificate(String studentId, String courseId) {
        if (courseId == null) {
            return;
        }
        ...
        // Check if student is eligible for certificate (all quizzes passed)
        if (certService.isStudentEligibleForCertificate(studentId, courseId)) {
            // Generate certificate automatically
            Certificate certificate = certService.generateCertificate(studentId, courseId);
            if (certificate != null) {
                System.out.println("Certificate generated: " + certificate.getCertificateId());
            }
        }
    }

    //find course id from a quiz
    private String getCourseIdFromQuiz(Quiz quiz) {
        ...
        for (Course course : courses) {
            for (Lesson lesson : course.getLessons()) {
                if (lesson.hasQuiz()
                        && lesson.getQuiz() != null
                        && lesson.getQuiz().getQuizId().equals(quiz.getQuizId())) {
